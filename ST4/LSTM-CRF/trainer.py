...
import time
from model import TransformersCRF
import torch
from transformers import *
from typing import List
from termcolor import colored
import os
from utils import get_metric, write_results, set_seed
import pickle
import tarfile
from tqdm import tqdm
from collections import Counter
from data import TransformersNERDataset
...
def parse_arguments(parser):
    ...
    parser.add_argument('--train_file', type=str, help="Location of Train File")
    parser.add_argument('--dev_file', type=str, help="Location of Validation File")
    parser.add_argument('--test_file', type=str, help="Location of Test File")
    ...
    parser.add_argument('--embedder_type', type=str, default="bert-base-cased",
                        choices=list(["bert-base-uncased", "bert-base-cased", "bert-large-uncased", "roberta-base", "roberta-large", "albert-base-v2"]),
...
def train_model(config: Config, epoch: int, train_loader: DataLoader, dev_loader: DataLoader, test_loader: DataLoader):
    ...
    print(colored(f"Using AdamW optimizeer by HuggingFace with {config.learning_rate} learning rate, "
                  f"eps: {1e-8}, weight decay: {0.0}, warmup_step: {0}, ", 'yellow'))
    ...
    scheduler = get_linear_schedule_with_warmup(
        optimizer, num_warmup_steps=0, num_training_steps=len(train_loader) * epoch
    )
...
def main():
    ...
    tokenizer = AutoTokenizer(conf.embedder_type).from_pretrained(conf.embedder_type)
