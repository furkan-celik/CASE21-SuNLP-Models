...
model = getGRUmodel(embedding_matrix, maxlen, PARAMS)
# model = getLSTMmodel(embedding_matrix, maxlen, PARAMS)
...
model.fit(X_train, y_train, validation_data= (X_val, y_val), batch_size= PARAMS["batch_size"], epochs= 100, callbacks= [
    ...
    tf.keras.callbacks.ModelCheckpoint(monitor = "val_auc", filepath = "./modelrnn", save_best_only = True, save_weights_only = True)
])

model.load_weights("./modelrnn")
